...
describe("server: delegation routes trust negatives (U6, release-blocking)", () => {
  ...
  it("thinker-unconfigured start runs on the worker slot (R5)", async () => {
    ...
    try {
      ...
      // notePath/baseHash are null in the start response: the server mints
      // them on the first successful /api/agent/notes create (plan 020 U5).
      expect(started.body.job.notePath).toBeNull();
      expect(started.body.job.baseHash).toBeNull();
...
describe("server: delegation reaches succeeded through the real /api/agent/notes seam", () => {
  // Plan 020 U5 regression: the delegate job writes through the agent-actor
  // /api/agent/notes route (durable Inbox note, journaled actor:agent). The
  // server picks the path under the configured Inbox and returns the
  // baseHash; the succeeded job's notePath must be readable via GET /api/note
  // and carry the delegated content.
  it("writes through /api/agent/notes, adopts the server path+baseHash, and serves the content", async () => {
    const dir = mkdtempSync(join(tmpdir(), "sinapso-delegate-seam-"));
    ...
    try {
      ...
      const { app: app3 } = createApp(graphPath, undefined, {
        configPath,
        openrouter: {
          fetch: (async (_url: string, init?: RequestInit) => {
            bodies.push(String(init?.body ?? ""));
      ...
      // Real ephemeral listener so the job's loopback fetch reaches the
      // actual /api/agent/notes route (supertest doesn't listen).
      ...
      const token = await fetch(`${base}/api/session`)
        .then((r) => r.json() as Promise<{ token: string }>)
        .then((b) => b.token);

      // Drive /api/delegate through the real socket so the route's
      // `http://${req.headers.host}` loopback base resolves to this server.
      const started = await fetch(`${base}/api/delegate`, {
        method: "POST",
        headers: {
      ...
      expect(startedBody.job.notePath).toBeNull();
      expect(startedBody.job.baseHash).toBeNull();

      // Poll status until the job reaches a terminal state through the real
      // /api/agent/notes write path.
      const deadline = Date.now() + 5_000;
      let final = startedBody.job;
      while (Date.now() < deadline) {
        const status = await fetch(
          `${base}/api/delegate/status?sessionId=s-seam`,
        ...
        final = statusBody.job;
        if (final.state === "succeeded" || final.state === "failed") break;
        await new Promise((r) => setTimeout(r, 25));
      }
      expect(final.state).toBe("succeeded");
      // The note path lands under the configured Inbox with the slug derived
      // from the delegate's default title ("Thinker document").
      expect(final.notePath).toMatch(/^inbox\/thinker-document(-\d+)?\.md$/);
      expect(final.baseHash).toMatch(/^[a-f0-9]{64}$/);
      expect(bodies[0]).toContain("meta/worker-model");

      // The server-generated path is readable through /api/note and carries
      // the delegated content; the journal entry is actor:agent (R5).
      const noteResp = await fetch(
        `${base}/api/note?id=${encodeURIComponent(final.notePath!)}&nolog=1`,
      );
      expect(noteResp.status).toBe(200);
      const note = (await noteResp.json()) as {
        id: string;
        markdown: string;
        baseHash: string;
      };
      expect(note.id).toBe(final.notePath);
      expect(note.markdown).toContain("Delegated");
      expect(note.baseHash).toBe(final.baseHash);
      const journal = readFileSync(
        join(dirname(graphPath), "changes.jsonl"),
        "utf-8",
      );
      const lastEntry = JSON.parse(journal.trim().split("\n").pop()!);
      expect(lastEntry.actor).toBe("agent");
      expect(lastEntry.action).toBe("create");
      expect(lastEntry.path).toBe(final.notePath);
    } finally {
...
describe("server: plan 021 manual Inbox Review", () => {
  const sessionToken = async (app: ReturnType<typeof createApp>["app"]) =>
    (await request(app).get("/api/session")).body.token as string;

  it("runs only on GET, returns local cards, and writes no state file", async () => {
    const { app, data, cleanup } = freshCatalogVault();
    try {
      await request(app).get("/api/inbox").expect(200);
      expect(existsSync(join(data, "inbox-review.json"))).toBe(false);
      const res = await request(app).get("/api/inbox/review").expect(200);
      expect(res.body.noteCount).toBe(1);
      expect(res.body.cards).toContainEqual(
        expect.objectContaining({
          action: "continue",
          note: expect.objectContaining({ path: "inbox/in-graph.md" }),
        }),
      );
      expect(existsSync(join(data, "inbox-review.json"))).toBe(false);
    } finally {
  ...
  it("requires a token for comments/dismissal and persists exact-card state", async () => {
    const { app, cleanup } = freshCatalogVault();
    try {
      const card = (await request(app).get("/api/inbox/review")).body.cards[0];
      await request(app)
        .put(`/api/inbox/review/${card.id}`)
        .send({ comment: "later" })
        .expect(403);
      const token = await sessionToken(app);
      await request(app)
        ...
        .send({ comment: "later" })
        .expect(200);
      const commented = await request(app).get("/api/inbox/review").expect(200);
      expect(commented.body.cards[0].comment).toBe("later");
      await request(app)
        .put(`/api/inbox/review/${card.id}`)
        .set("x-sinapso-token", token)
        .send({ state: "dismissed" })
        .expect(200);
      const dismissed = await request(app).get("/api/inbox/review").expect(200);
      expect(
        dismissed.body.cards.some(
  ...
  it("returns stale without writing when source bytes changed", async () => {
    const { app, vault, cleanup } = freshCatalogVault();
    try {
      const card = (await request(app).get("/api/inbox/review")).body.cards[0];
      writeFileSync(
        join(vault, "inbox", "in-graph.md"),
        "changed externally\n",
      );
      const res = await request(app)
        .post(`/api/inbox/review/${card.id}/apply`)
        .set("x-sinapso-token", await sessionToken(app))
        .send({})
        .expect(200);
      expect(res.body).toMatchObject({ error: "review-stale", stale: true });
      expect(readFileSync(join(vault, "inbox", "in-graph.md"), "utf8")).toBe(
        "changed externally\n",
      );
    } finally {
      cleanup();
  ...
  it("merges into the target and preserves the Inbox source", async () => {
    const { app, vault, cleanup } = freshCatalogVault();
    try {
      writeFileSync(
        join(vault, "loose.md"),
      ...
      const review = await request(app).get("/api/inbox/review").expect(200);
      const card = review.body.cards.find(
        (item: { action: string }) => item.action === "merge",
      );
      expect(card.preview).toContain("## Merged from shared");
      await request(app)
        .post(`/api/inbox/review/${card.id}/apply`)
        .set("x-sinapso-token", await sessionToken(app))
        .send({})
        .expect(200);
      expect(readFileSync(join(vault, "loose.md"), "utf8")).toContain(
        "## Merged from shared",
      );
      expect(
        readFileSync(join(vault, "inbox", "in-graph.md"), "utf8"),
      ).toContain("Source");
    } finally {
      cleanup();
    }
  ...
  it("archives an empty note with a guarded move", async () => {
    const { app, vault, cleanup } = freshCatalogVault();
    try {
      writeFileSync(join(vault, "inbox", "empty.md"), "  \n");
      const review = await request(app).get("/api/inbox/review").expect(200);
      const card = review.body.cards.find(
        (item: { action: string; note: { path: string } }) =>
          item.action === "archive" && item.note.path === "inbox/empty.md",
      );
      const applied = await request(app)
        ...
        .send({})
        .expect(200);
      expect(applied.body.resultPaths).toEqual(["archive/empty.md"]);
      expect(existsSync(join(vault, "inbox", "empty.md"))).toBe(false);
      expect(existsSync(join(vault, "archive", "empty.md"))).toBe(true);
    } finally {
      cleanup();
    }
  });

  it("uses only a fresh cached semantic edge for a CAS-safe Link apply", async () => {
    const { app, data, vault, cleanup } = freshCatalogVault();
    try {
      writeFileSync(
        ...
        JSON.stringify({
          ...
          count: 1,
          edges: [
            { source: "inbox/in-graph.md", target: "loose.md", score: 0.8 },
          ],
        }),
      );
      const review = await request(app).get("/api/inbox/review").expect(200);
      const card = review.body.cards.find(
        (item: { action: string }) => item.action === "link",
      );
      expect(card).toMatchObject({
        action: "link",
        target: { path: "loose.md" },
      });
      await request(app)
        .post(`/api/inbox/review/${card.id}/apply`)
        .set("x-sinapso-token", await sessionToken(app))
  ...
  it("offers Ingest only with an enabled wiki and apply returns a handoff without provider work", async () => {
    const { app, vault, cfgPath, cleanup } = freshCatalogVault();
    try {
      ...
      const review = await request(app).get("/api/inbox/review").expect(200);
      const card = review.body.cards.find(
        (item: { action: string }) => item.action === "ingest",
      );
      const before = readFileSync(join(vault, "inbox", "in-graph.md"), "utf8");
      const applied = await request(app)
        .post(`/api/inbox/review/${card.id}/apply`)
        .set("x-sinapso-token", await sessionToken(app))
        .send({})
        .expect(200);
      expect(applied.body.handoff).toBe("wiki-ingest");
      expect(readFileSync(join(vault, "inbox", "in-graph.md"), "utf8")).toBe(
        before,
      );
    } finally {
  ...
  it("returns an explanatory empty review for an excluded Inbox", async () => {
    const { app, cleanup } = freshCatalogVault({
      writeDestination: "Private",
      adminExcludes: ["Private"],
    });
    try {
      const review = await request(app).get("/api/inbox/review").expect(200);
      expect(review.body).toMatchObject({ noteCount: 0, cards: [] });
    } finally {
      cleanup();
    }
  });
