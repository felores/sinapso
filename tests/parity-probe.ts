...
const TOOL = {
  name: "start_report",
  description: "Starts generating a background report. Call it when the user asks for a report.",
...
async function probe(voice: string): Promise<string> {
  return new Promise<string>((resolve) => {
    ...
    let done = false;
    let sess: any = null;
    const finish = (msg: string) => { if (!done) { done = true; try { sess?.close(); } catch {} resolve(msg); } };
    const timer = setTimeout(() => finish(`TIMEOUT phase=${phase} audio=${audioBytes}`), 30000);
    ai.live.connect({
      model,
      config: {
      ...
      callbacks: {
        ...
        onmessage: (m: any) => {
          ...
          if (m?.setupComplete) {
            ...
            sess?.sendClientContent({ turns: [{ role: "user", parts: [{ text: asyncMode ? "Please start a report about the ocean using your tool." : "Say the word OK and nothing else." }] }], turnComplete: true });
          }
          ...
          if (calls?.length && phase === "awaiting_call") {
            ...
            sess?.sendToolResponse({ functionResponses: [{ id: fc.id, name: fc.name, response: { result: "started" } }] });
            setTimeout(() => {
              phase = "awaiting_final";
              audioBytes = 0; // count only post-completion audio
              sess?.sendToolResponse({ functionResponses: [{ id: fc.id, name: fc.name, response: { result: "The report is finished. Tell the user it is ready.", scheduling: FunctionResponseScheduling.INTERRUPT } }] });
            }, 6000);
          }
          if (m?.serverContent?.turnComplete && phase === "awaiting_final" && audioBytes > 0) {
            clearTimeout(timer);
            finish(`PASS audio=${audioBytes}`);
          }
        },
        onerror: (e: any) => { clearTimeout(timer); finish(`ERROR ${String(e?.message ?? e).slice(0, 140)}`); },
        onclose: (e: any) => { clearTimeout(timer); if (!done) finish(`CLOSED ${String(e?.reason ?? "").slice(0, 140)}`); },
      },
    }).then((s) => { sess = s; }).catch((e) => { clearTimeout(timer); finish(`CONNECT_FAIL ${String(e?.message ?? e).slice(0, 140)}`); });
  });
}

async function main() {
  for (const v of voices) console.log(`${model}${asyncMode ? " [async-fc]" : ""} ${v}: ${await probe(v)}`);
}
void main();
