...
updateConfig({
  ...
  voice: { provider: "gemini", voice: "Charon", keys: { gemini: real.voice.keys.gemini } },
}, configPath);
...
const loggingFetch = (async (input: unknown, init?: RequestInit) => {
  if (String(input).includes("/chat/completions") && init?.body) {
    try { llmModels.push(JSON.parse(String(init.body)).model); } catch {}
  }
...
function pcmChunks(wavPath: string): string[] {
  ...
  const chunks: string[] = [];
  for (let i = 0; i < buf.length; i += 6400) // 200ms @16kHz mono 16-bit
...
const done = new Promise<void>((resolve) => {
  ...
  ws.on("message", (data) => {
    ...
    if (m.type === "audio") { audioBytes += m.data.length; if (completionSeen) audioAfterCompletion += m.data.length; return; }
    events.push(JSON.stringify(m).slice(0, 160));
    if (m.type === "status" && m.key === "voice.status.delegating") delegated = true;
    if (m.type === "action" && m.action === "open_research") { openedResearch = true; completionSeen = true; }
    if (m.type === "error") { console.error("WS error:", m.message); clearTimeout(timer); resolve(); }
    if (openedResearch && audioAfterCompletion > 5000) { clearTimeout(timer); setTimeout(resolve, 3000); }
  });
...
).json().catch(() => ({}))) as { entries?: Array<{ mode: string; document?: { title?: string } }> };
ws.close();
...
console.log("audio after completion (spoken heads-up bytes):", audioAfterCompletion);
...
server.close();
process.exit(delegated && openedResearch && llmModels.includes("deepseek/deepseek-v4-flash") ? 0 : 1);
