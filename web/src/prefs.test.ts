...
describe("prefs: namespace guard", () => {
  it("writes only akasha-* keys (covers every accessor)", () => {
    ...
    for (const k of Object.keys(storage.entries())) {
      expect(k.startsWith("akasha-")).toBe(true);
    }
    // every akasha-* key in the inventory was written
    for (const k of ALL_KEYS) {
      expect(storage.written.has(k), `expected write to ${k}`).toBe(true);
    }
  });

  it("remove operations stay inside akasha-*", () => {
    prefs.removeColors();
    prefs.removePc();
    prefs.removeMode();
...
describe("prefs: numeric keys", () => {
  it("depth: default 2, round-trips as string", () => {
    ...
    expect(storage.getItem("akasha-depth")).toBe("3");
  });

  it("depth: non-numeric stored value falls through to default (NaN→2)", () => {
    storage.setItem("akasha-depth", "garbage");
    expect(prefs.getDepth()).toBe(2);
  });
  ...
  it("ui-zoom: default 1, round-trips", () => {
    ...
    expect(storage.getItem("akasha-ui-zoom")).toBe("1.4");
  });

  it("ui-zoom: missing or NaN falls back to 1", () => {
    expect(prefs.getUiZoom()).toBe(1);
    storage.setItem("akasha-ui-zoom", "not-a-number");
    expect(prefs.getUiZoom()).toBe(1);
  });
});
...
describe("prefs: pc (particle count) — nullable", () => {
  ...
  it("setPc stores a number string; removePc deletes the key", () => {
    prefs.setPc(2048);
    expect(storage.getItem("akasha-pc")).toBe("2048");
    expect(prefs.getPc()).toBe("2048");
    prefs.removePc();
    expect(prefs.getPc()).toBeNull();
    expect(storage.getItem("akasha-pc")).toBeNull();
  });
});
...
describe("prefs: mode (vault|web|ingest|null)", () => {
  ...
  it("setMode writes a value, removeMode clears it", () => {
    prefs.setMode("web");
    expect(storage.getItem("akasha-mode")).toBe("web");
...
describe("prefs: web-scope (deep|web)", () => {
  it("default 'deep'; non-'web' stored values still read as 'deep'", () => {
    ...
    storage.setItem("akasha-web-scope", "garbage");
    expect(prefs.getWebScope()).toBe("deep");
    prefs.setWebScope("web");
...
describe("prefs: JSON keys — colors", () => {
  ...
  it("set then get preserves parse/stringify identity", () => {
    ...
    expect(JSON.parse(storage.getItem("akasha-colors") as string)).toEqual(c);
  });
  ...
  it("removeColors deletes the key", () => {
    ...
    prefs.removeColors();
    expect(prefs.getColors()).toEqual({});
    expect(storage.getItem("akasha-colors")).toBeNull();
  });
});
...
describe("prefs: JSON keys — filters (try/catch → [] on corrupt)", () => {
  ...
  it("set then get preserves parse/stringify identity", () => {
    ...
    expect(JSON.parse(storage.getItem("akasha-filters") as string)).toEqual(f);
  });

  it("corrupt JSON falls back to [] exactly like the inline try/catch", () => {
    storage.setItem("akasha-filters", "{not valid");
    expect(prefs.getFilters()).toEqual([]);
  });

  it("non-array JSON falls back to [] exactly like the inline isArray check", () => {
    storage.setItem("akasha-filters", JSON.stringify({ a: 1 }));
    expect(prefs.getFilters()).toEqual([]);
  });
});
...
describe("prefs: JSON keys — reader geometry (akasha-reader)", () => {
  ...
  it("setReader round-trips and stores JSON.stringify", () => {
    ...
    expect(JSON.parse(storage.getItem("akasha-reader") as string)).toEqual(g);
  });
});

describe("prefs: JSON keys — research geometry (akasha-research)", () => {
  it("default null when key missing; corrupt JSON throws", () => {
    expect(prefs.getResearch()).toBeNull();
    storage.setItem("akasha-research", "{not valid");
    expect(() => prefs.getResearch()).toThrow();
  });

  it("'null' sentinel parses to null and the default kicks in", () => {
    storage.setItem("akasha-research", "null");
    expect(prefs.getResearch()).toBeNull();
  });
...
describe("prefs: default storage", () => {
  it("createPrefs() with no args uses window.localStorage", () => {
    ...
    try {
      const p = createPrefs();
      p.setTheme("cosmos");
      expect(localMock.setItem).toHaveBeenCalledWith("akasha-theme", "cosmos");
      p.getTheme();
...
describe("prefs: bytes-on-the-wire identity (catch regressions vs current storage)", () => {
  it("writes the exact strings current main.ts writes", () => {
    ...
    expect(storage.getItem("akasha-phantoms")).toBe("1");
    expect(storage.getItem("akasha-orphans")).toBe("0");
    expect(storage.getItem("akasha-glow")).toBe("0");
    expect(storage.getItem("akasha-sem-lines")).toBe("0");
    expect(storage.getItem("akasha-labels")).toBe("0");
    expect(storage.getItem("akasha-arrangement")).toBe("semantic");
    expect(storage.getItem("akasha-quality")).toBe("low");
    expect(storage.getItem("akasha-nodes")).toBe("starlight");
    expect(storage.getItem("akasha-depth")).toBe("1");
    expect(storage.getItem("akasha-ui-zoom")).toBe("0.8");
