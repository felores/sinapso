...
- R7. The edited source node is the only movable retained node during the local adjustment. Existing reference nodes and all unrelated nodes remain fixed at their current positions. A newly created phantom target remains fixed at its seeded neighbor position. A source is pre-fixed only when `fx != null || fy != null || fz != null`; such a source skips physics and camera follow rather than overriding the user's fixed state. Present `null` or `undefined` fixed-coordinate properties do not prevent movement.
...
- R14a. When a graph fingerprint changes, every arrangement-memory entry from the previous fingerprint is discarded. The active arrangement is repopulated only from its newly settled or intentionally stationary positions; switching arrangements can never restore positions from the previous topology.
- R15. Structural graph responses carry their local save-request ordinal. Only a newer accepted graph-bearing response supersedes an older graph-bearing response across reader and research editors. A later prose-only save does not suppress a valid structural graph response. Editor teardown disables relayout and camera follow for that editor but still permits an accepted structural response to update topology without `liveSourceId`; vault/app teardown invalidates all pending graph application.
- R16. A post-write scan failure never rolls back or reports failure for the already-durable Markdown edit. The response returns the new `baseHash` plus `graphRefreshFailed: true`; the current client presentation graph and camera remain unchanged. No rollback of an already atomically written `graph.json` is promised if a subsequent scan-cache write or reload step fails.
...
- KTD8. **One live-relayout lifecycle.** Store one small relayout record containing source id, previous source position, fixed-coordinate snapshots, follow state, and prior warmup/cooldown settings. Set `warmupTicks(0)` before live `graphData()` so no unobserved synchronous movement occurs before camera tracking. Cleanup restores both settings, is idempotent, and always precedes replacement. A changed graph fingerprint clears all `arrLayoutMem` entries before the active arrangement is repopulated after settling or stationary application.
- KTD9. **Order graph responses, not ordinary saves.** Reader and research autosaves share monotonically increasing request ordinals, while `latestAppliedGraphOrdinal` advances only when a graph-bearing response is accepted. A response applies topology when its ordinal is newer than the latest accepted graph ordinal. Editor teardown removes live-motion eligibility for its request but does not discard valid topology; vault/app teardown invalidates the request entirely.
...
- **Work:** Extend reader and research vault-note save response handling. Assign every save a shared monotonically increasing request ordinal. When a response contains `graph`, accept it only when its ordinal exceeds `latestAppliedGraphOrdinal`, then advance that accepted ordinal and call `applyGraphUpdate()`. Pass `liveSourceId: noteId` only while the originating editor still owns that note and remains eligible for motion; otherwise apply topology without motion. Prose-only responses never advance the accepted graph ordinal. Editor teardown removes motion eligibility without cancelling the durable request; vault/app teardown invalidates pending graph application. Reuse the existing localized graph-refresh error when `graphRefreshFailed` is true. Keep keepalive/unload saves presentation-free.
- **Tests:** Reader and research autosave apply structural graph responses; prose response does not call graph update or suppress an older structural response; an older graph-bearing response cannot replace a newer accepted graph; editor teardown permits topology but not motion; vault/app teardown rejects late graph application.

### U4. Local graph relayout lifecycle

- **Depends on:** U3.
- **Files:** `web/src/types.ts`, `web/src/main.ts`.
- **Work:** Add optional fixed-coordinate fields to `GNode`. Extend `applyGraphUpdate()` with the optional live source id. Before `graphData()`, clean any old relayout, treat only non-nullish source fixed coordinates as pre-fixed, snapshot/pin unaffected nodes, recompute the active arrangement edge set and selected BFS focus, and rerun the same degree-based structural/semantic link-strength configuration used by `applyArrangement()`. On fingerprint change, clear every arrangement-memory entry before applying the graph. Snapshot warmup/cooldown, set warmup to zero plus bounded cooldown ticks, and defer active-arrangement repopulation and `saveLayout()` until engine stop. Stationary paths immediately repopulate only the active arrangement from current positions. For non-live same-fingerprint updates preserve the existing zero-tick path exactly.
- **Tests:** Pure property-presence/value snapshot and restore assertions where practical; `null`/`undefined` source coordinates remain movable while numeric fixed coordinates stay fixed; browser proof that source coordinates change only through observed engine ticks, unrelated coordinates stay within tolerance, Hybrid uses refreshed degrees, the new edge exists, cleanup removes temporary pins and restores warmup/cooldown, and arrangement switching cannot restore old-fingerprint memory.
...
- AE7. In Semantic arrangement, reduced-motion mode, or when the source has a non-nullish pre-existing fixed coordinate, the edge appears but node and camera positions remain stable.
