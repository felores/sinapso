...
- **Objective:** Add safe Git status, commit, and sync UX to Solaris's Admin modal, including fetch, fast-forward inbound updates, and push to the current branch's upstream.
...
- **Execution profile:** Server + frontend. Security-sensitive: all mutating routes are token-guarded; git commands run through `execFile`/`Runner` with no shell strings, no `--all`, no `git reset --hard`/`git checkout --`, no amend, no force push. Sync is a new explicit Git contract exception to the single-writer rule: a user-triggered, clean-tree, fast-forward-only repo operation may update vault files through Git.
- **Stop conditions:** Stop and ask before adding rebase, non-fast-forward merge, stash/unstash, conflict resolution, bisect, submodule support, commit message editing beyond the one text input, or anything that modifies files outside Git's fast-forward update. Stop before any automatic commit of scanner output (graph.json, data/).
...
Solaris already has read-only Git version history for individual notes in the reader. When the vault is inside a Git repo, the Admin modal should surface vault-level Git status: dirty/clean indicator, file count, and action buttons to stage+commit (keyboard-simple: one message input + commit button) and sync the current branch with its upstream. Sync handles the safe common cases automatically: fetch, fast-forward inbound changes when the local branch is only behind, push local commits when the local branch is only ahead, and stop on divergence/conflicts. The vault's `.gitignore` must be respected (no forced adds). The feature is purely optional: vaults without Git repos show nothing.

### Requirements

- **R1.** Admin modal shows a "Git" section with vault-level status (clean/dirty, staged/untracked count) when the vault is in a Git repo.
- **R2.** Commit flow: one text input for message + one "Commit" button. Stages all changed files under the vault path (respecting `.gitignore`), then commits. Reports success or failure inline.
- **R3.** Sync flow: one "Sync" button that requires a clean working tree, resolves the current branch's upstream, fetches it, fast-forward-merges only when safe, then pushes local commits when needed. Divergence/conflicts/no-upstream stop with a clear message. Never runs `git pull`, non-fast-forward `git merge`, `git rebase`, or conflict resolution.
...
- **F1. View Git status in Admin.** Admin modal resolves the vault repo's status (dirty staged/untracked/changed files, branch name, ahead/behind counts). Renders a `.admin-section` with branch name, dirty/clean badge, file list, commit input, commit and sync buttons. Non-Git vault → section absent entirely.
- **F2. Commit changes.** User types a message, clicks Commit. POST to token-guarded `/api/git/commit` with `{ message }`. Server stages vault-relative changed files (`git add -- <paths>`) and commits (`git commit -m <msg>`). Returns `{ ok: true }` or error. Admin refreshes status.
- **F3. Sync (fetch + fast-forward + push).** User clicks Sync. POST to token-guarded `/api/git/sync`. Server refuses dirty trees, resolves the actual upstream, fetches it, computes ahead/behind, stops if both ahead and behind, runs `git merge --ff-only <upstream>` when behind, then runs `git push <upstream-remote> HEAD:<upstream-branch>` when ahead. Returns `{ ok: true, output: "..." }`. On failure returns `{ ok: false, error: "..." }` (dirty tree, divergence/conflict, no upstream).

### Acceptance Examples

- **AE1.** Vault with no `.git` repo → Admin modal shows no Git section.
- **AE2.** Vault with a Git repo and no changes → Git section shows "clean", branch name, ahead/behind. Commit input + buttons present but commit disabled if nothing staged.
- **AE3.** Modify a tracked file, open Admin → dirty status shown, file listed.
- **AE4.** Type "fix typo" and click Commit → server stages paths under vault, commits, returns ok, Admin refreshes to clean.
...
- **AE8.** Click Sync with divergent history → error shown, working tree unchanged.
...
- Admin-modal Git section: status, branch, dirty/clean, staged/untracked file list.
- One-line commit input + commit button (stages vault paths only).
- Sync button (fetch + fast-forward + push; stops on divergence/conflicts).
...
- Conflict resolution or non-fast-forward merge.
- Rebase-based sync.
...
- Contract update target: `CLAUDE.md` conventions and `git.ts` architecture bullet.
...
- **KTD3.** Sync = clean-tree check, resolve upstream (`@{u}` plus remote/merge config), `git fetch <remote> <branch>`, ahead/behind check, `git merge --ff-only <upstream>` only if behind, then `git push <remote> HEAD:<branch>` only if ahead. If the tree is dirty, fetch fails, or ahead/behind indicates divergence, return error and skip mutation beyond fetch. No `git pull`, no `--rebase`, no force push.
- **KTD4.** Two new routes: `POST /api/git/commit` (guarded, body `{ message }`) and `POST /api/git/sync` (guarded, empty body). `GET /api/git/status` is read-only and has no token guard.
- **KTD5.** Admin UI: new `<section class="admin-section">` between wikis and prompts. Shows branch badge, dirty badge, file table (small), commit input + button, sync button. Status span for operation feedback. Non-Git vault → not rendered; `GET /api/git/status` returns `{ available: false }`.
...
- **KTD7.** Update the repo contract in `CLAUDE.md`: app-authored note writes still go through `write.ts`; user-triggered Git sync is the only sanctioned repo-level exception and lives in `git.ts`, token-guarded, clean-tree, fast-forward-only, no checkout/reset/rebase/force.
...
- **Approach:**
  ...
  - Add `gitSync(run, repoRoot)`: refuse dirty working trees, resolve upstream, fetch the upstream remote/branch, compute ahead/behind, stop if both are non-zero, fast-forward merge if behind, push if ahead, and return combined output.
...
- **Test scenarios:** porcelain parsing for clean/dirty/modified/untracked; stage respects gitignore; commit writes object; sync fetch+ff succeeds when behind; sync push succeeds when ahead; sync refuses dirty tree; sync with divergence returns error before merge/push; empty message rejected; sync without upstream returns error; non-`origin` upstream works.
...
- **Approach:**
  ...
  - `POST /api/git/commit` (guarded, express.json): body must have `message` string (non-empty, trimmed). Resolve vault root relative paths for files under vault, call `gitStage` + `gitCommit`. Return `{ ok: true }` or 400/500 error.
...
- **Test scenarios:** non-git vault returns `available:false`; commit with token succeeds; commit without token 403; sync with upstream succeeds; sync without remote/tracking returns error; commit with empty message 400; status after commit shows clean; sync without token 403.
...
- **Approach:**
  ...
  - Add `renderGitSection()`: calls `GET /api/git/status`. If `available: false`, set innerHTML to `""` (section hidden/removed). If available, render branch badge, clean/dirty badge, file list (compact table or list), commit message `<input>` + commit button, sync button.
  - Add `refreshGitSection()` called after commit/sync to re-fetch and re-render.
  - Wire commit button click: POST `/api/git/commit` with input value, disable button during flight, flash status message.
  - Wire sync button click: POST `/api/git/sync`, show output in an inline `<pre>` result area, flash status.
  - Add i18n keys for "git", "gitBranch", "gitClean", "gitDirty", "gitCommit", "gitCommitPlaceholder", "gitSync", "gitSyncing", "gitCommitted", "gitSynced", "gitError", "gitNoRemote".
  - CSS: git section file list uses small `.git-file-row` with monospace status badge. Buttons styled as `.maint-action-btn` or reuse `.ghost` pattern.
...
- Open Admin on git vault with clean status → shows branch, "clean" badge, commit input, sync button.
- Modify a tracked file → Admin shows dirty, file listed.
- Type commit message and click Commit → success flash, section refreshes clean.
...
- Admin modal shows a Git section with status, commit input+button, sync button.
...
- Sync is fetch + `--ff-only` only; conflicts and divergence return clear errors without changing the working tree.
