...
- R8. If the note changed on disk since it was loaded (external edit, git sync/restore, or another in-app writer such as voice promote, wiki-ingest apply, or MCP edit), autosave must not silently clobber it — the UI warns and offers reload or overwrite. Discovery is save-time (the conflicting save is rejected), not live change detection. The conflict prompt is a non-blocking banner: the editor stays live and dirty while it is open.
- R9. A subtle save-state indicator shows dirty/saving/saved/error, distinct from the existing git working-tree indicator. A non-409 save failure (network drop, 500, exhausted token retry) shows the error state visibly and leaves the editor dirty for retry, matching the 403 recovery posture.
...
- R14. The editable body preserves assistive-tech navigability of headings and links at parity with the current sanitized-HTML reader; the migration to a decoration-rendered editor must not regress screen-reader navigation.
...
stateDiagram-v2
  ...
  note right of Saving: single-flight — triggers arriving here set one pending flag; at most one follow-up PUT after resolve
  Saving --> Clean: 200 (saved content becomes base)
  Saving --> Dirty: 200 with pending flag (follow-up PUT with promoted base)
  Saving --> Conflict: 409 (disk hash differs from baseHash)
  Saving --> Error: non-409 failure (indicator shows error, editor stays dirty)
  Error --> Saving: next flush retries
...
- **Test scenarios:**
  - Round-trip byte equality for fixtures covering: YAML frontmatter, `[[wiki-links]]` (incl. aliased `[[target|alias]]`), task lists, nested lists, code fences with language tags, embedded raw HTML, consecutive blank lines, trailing whitespace/newline, unicode/accented text, and a CRLF-line-ending note that round-trips byte-for-byte (no LF normalization).
  - Select-all-and-retype over a note with folded frontmatter: the body is replaced, the frontmatter block is byte-identical (KTD3 transaction filter).
...
- **Requirements:** R1, R2, R4, R5, R10, R13, R14.
- **Dependencies:** U1.
...
- **Approach:** `autosave.ts` owns the state machine from the HTD diagram: base content captured at load, dirty on divergence, debounce on idle, flush on blur/note-switch, byte-compare suppression, single-flight per KTD4 (pending flag while a save is in flight, one follow-up PUT with the promoted base); on 200 the saved content becomes the new base; non-409 failures land in the error state (indicator visible, editor dirty, next flush retries). Saves go through `api()` with explicit `method: "PUT"` (auto-attaches `x-solaris-token`). Unload safety per KTD4b: `keepalive` PUT attempt plus `localStorage` dirty mirror via `prefs.ts` (vault-scoped: mirror stores the active vault path; restore only offered on vault match, non-matching mirrors discarded), with restore-on-next-open. Server per KTD5/KTD5b: `guardedEdit` gains optional `baseHash` (409 on mismatch), skips write+journal when incoming content equals disk, and writes atomically via symlink-aware temp+rename. Existing callers pass no `baseHash` and are unaffected. Conflict UI is a non-blocking banner per R8 (editor stays live and dirty while it is open) offering reload (re-set editor from disk) or overwrite (retry without `baseHash`). The `baseHash` algorithm/encoding lives in one shared definition per KTD5. New dirty flag named distinctly (e.g., `editorDirty`) — `readerNoteDirty` already means git-working-tree state.
...
- **Test scenarios:**
  ...
  - Single-flight: a blur flush firing while a debounce save is in flight yields exactly two total PUTs (in-flight + one follow-up with the promoted base) and no 409; a burst of triggers during Saving collapses to one pending follow-up.
  - Non-409 failure (network error, 500): indicator shows the error state, editor stays dirty, next flush retries.
  - Unload safety: dirty content is mirrored to the `prefs.ts` key on close with the active vault path; reopening that note in the same vault offers restore; a mirror from a different vault is discarded, never offered; mirror cleared on successful save.
...
| Manual | `npm run dev` | Look parity across themes, toolbar positioning, dock/float geometry, conflict banner, and an accessibility spot-check (R14: heading/link navigation with VoiceOver or equivalent) — DOM behavior Vitest can't cover |
