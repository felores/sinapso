...
## U10 completion — Pure frontend modules: filters + clusters

**Features:**
- F059: not_started -> passing. New `web/src/types.ts` (smallest type move): `GNode` + `GLink` lifted from `main.ts`. New `web/src/filters.ts` (45 lines) exports `subsequence`, `filterFields(n, groupOf)`, `compileMatcher(pattern, groupOf)` — `groupOf` passed in as second arg to keep them pure per KTD6; bodies moved verbatim. New `web/src/clusters.ts` (118 lines) exports `computeSemanticClusters(nodes, semanticLinks) -> Map<string, string>` — label-propagation body lifted verbatim with internal `byId` (closure's `endNode()` + `byId.get()` collapsed into one local resolver handling `string | GNode`). `web/src/main.ts` shrank -134 / +10: local `GNode`/`GLink` types deleted (imported from `./types`); `subsequence`/`filterFields`/`compileMatcher` bodies deleted; the closure's `computeSemanticClusters` is now a 7-line thin composition that calls the pure helper and assigns each `id -> name` to `clusterNameOfId`. The call site of `compileMatcher` in `applyFilters` passes `groupOf` as the second arg. `activeChain` stays in `boot()` as thin composition per plan. 40 new tests (filters 28: subsequence hit/miss/empty/equal-hay/empty-hay; filterFields lowercased tuple + missing title/group/tags handling + groupOf wiring; compileMatcher empty-whitespace predicate, plain substring on all fields, plain subsequence fallback title-only at len>=3, len<3 gate, case-insensitivity, wildcards `*`->`.*` / `?`->`.` / anchored end-to-end, full regex-metachar escape set `[]() +$^|\`, plain pattern against groupOf; clusters 12: empty edge set, two disconnected cliques distinct, deterministic across runs, singleton nodes absent, empty nodes array, tag-named cluster, pillar fallback, "cluster" fallback, dedupe with numeric suffix, GLink.source/target as GNode shape, asymmetric link set, stability across input array orders).

**Verifier result:** `npm test` 440/440 pass (31 files; +40 vs prior 400: `web/src/filters.test.ts (28 tests)`, `web/src/clusters.test.ts (12 tests)`); `npm run typecheck` clean (0 errors); `npm run build` green (vite dist 1.54 MB / 426 KB gzipped, unchanged envelope). Behavior discrepancies accepted as current behavior preserved (no UI change): (1) `groupOf` now a second parameter on `filterFields`/`compileMatcher` rather than captured from closure — KTD6's "receive what you read from closure as parameters" outranks the plan prose's "verbatim one-arg"; main.ts passes the same closure function so observable behavior is identical. (2) `computeSemanticClusters` prepends `#` to the raw tag value, so tag `#g` yields cluster name `##g` — existing code, tests reflect it. (3) plain substring match scans every field (title/group/tags); the fuzzy subsequence fallback applies to the title (first field) only when `pattern.length >= 3` — derived from current code. (4) manual UI smoke (type a filter in the search row, toggle `group by: semantic cluster`) deferred — no browser in this environment; verifier accepted on the test + build evidence per R10.

## U11 completion — Frontend api() module + migration

**Features:**
- F060: not_started -> passing. New `web/src/api.ts` (95 lines) exports `api(path, opts?)` (GET default; `{json}` triggers POST with `content-type: application/json` + `x-solaris-token`; `opts.method` overrides default; token memoized internally with a single inflight promise so concurrent first-mutating calls share one `/api/session` fetch; non-OK throws `ApiError` carrying `status` and parsed body; 204/empty body returns `null`; non-JSON error body kept as text), `apiRaw(path, init?)` (preserves fetch semantics; `init.token === true` injects the session token into caller-supplied `Headers` for non-JSON uploads), `ApiError` (status, body), `getApiToken` (module-internal token fetch + memo), `resetApiToken` (test seam). New `web/src/api.test.ts` (294 lines, 22 tests) with a stubbed global `fetch` reset in `beforeEach` via `resetApiToken()`. `web/src/main.ts` shrank +128 / -234 net -106 lines. Absorbed `apiToken`/`sessionToken` (now `getApiToken` in api.ts); the only remaining token reference in main.ts is the voice session start at line 3319 (`startVoice(await getApiToken(), ...)`) which is in-scope only as a call site — the voice module's own WebSocket/token use is out of scope per task. `postConfig` is retained as a 1-line arrow function delegating to `api("/api/integrations/config", { json: patch })`; the wrapper no longer hand-builds `x-solaris-token`, and the token/config logic is absorbed into api.ts via `getApiToken`. Step 0 classification: 38 sites (a) — res.ok or try/catch/`.catch` already absorbs the `ApiError` — migrated to `api()`; 1 site (b) the boot graph fetch `/api/graph` (line 122) has no surrounding handler and currently continues on 4xx via `.json()` parsing — migrated to `apiRaw` to preserve that behavior per KTD4; 1 site (c) the octet-stream upload `/api/wiki-ingest/propose-upload` or `/api/ingest-upload` (line 4962) — migrated to `apiRaw` with `token: true` so the caller's `content-type: application/octet-stream` and binary `body` are preserved. Behavior preservation: `startMaint` (line 3626) wraps the `api()` call in an inner `try/catch` to preserve the previous `if (!res.ok && res.status !== 409)` tolerance (409 still treated as success). `runWebQuery` (line 4632) and `runArticleFetch` (line 4667) extract `data.message` from `ApiError.body` so the string shown by `researchError` matches the previous message. No new error surfacing anywhere.

**Verifier result:** `npm test` 462/462 pass (32 files; +22 new vs prior 440: `web/src/api.test.ts`); `npm run typecheck` clean (0 errors); `npm run build` green (vite dist 1,542.50 kB / 426.86 kB gzipped). Grep counts: `grep -cE "\bfetch\(" web/src/main.ts` -> 0 (was 40); `grep -c "x-solaris-token" web/src/main.ts` -> 0 (was 17 hand-builds + 1 in old `postConfig` definition); `grep -cE "apiToken|sessionToken" web/src/main.ts` -> 0; `grep -c "postConfig" web/src/main.ts` -> 13 (1 definition + 12 call sites; the definition is a 1-line `api()` wrapper, not a hand-built token); `x-solaris-token` / `fetch(` inside `web/src/api.ts` -> 2 each (both inside the module, never leak out). Spirit-met discrepancy (low severity): the named `postConfig` wrapper is retained as a 1-line `api(...)` delegate; KTD4's plan prose specifies absorbing `apiToken`/`sessionToken` and does not list `postConfig`, so the wrapper is the most conservative migration — no call site hand-builds the token header, the token/config logic is in api.ts. Manual UI smoke (save-note, rescan, semantic search, wiki-ingest proposal round-trip, orphan-link confirm) deferred — no browser in this environment; verifier accepted on test + build + grep evidence per R10. Stash@{0}/stash@{1}, `docs/plans/2026-07-07-010-feat-text-selection-to-voice-plan.md`, and `.scratchpad/` untouched.

## U12 completion — Typed prefs module + migration

**Features:**
- F061: not_started -> passing. New `web/src/prefs.ts` (~286 lines) exports `createPrefs(storage?)` returning a typed `Prefs` facade. Constructor takes a `PrefsStorage`-like `{getItem, setItem, removeItem}` defaulting to `window.localStorage`. 26 accessors, one per existing `akasha-*` key: theme / group / colors / phantoms / orphans / depth / quality / glow / arrangement / semLines / filters / labels / nodes / sizeWeights / pc / reader / research / mode / webScope / qmdPrompted / uiZoom / labelDistance / labelSize / nodeSize / linkOpacity / minWeight. The 5 dynamic-slider keys share a private `numPref(key)` helper but each is its own public accessor pair. Type enums re-exported (`ModeName`, `GroupMode`, `QualityKey`, `NodeStyle`, `Arrangement`, `WebScope`, `Filter`, `SizeWeights`, `ReaderGeom`, `ResearchGeom`). `web/src/main.ts` net change: 56 `localStorage` call sites replaced with prefs accessors. The 3 "Persisted in localStorage" comments updated to "Persisted via prefs". `bindRange` signature extended with `rw: {read, write}` argument; the 5 call sites supply `{read: prefs.getXxx, write: prefs.setXxx}`. Key names and stored formats byte-identical to pre-unit: theme writes the raw string, "1"/"0" flags write that literal, JSON keys go through `JSON.parse` / `JSON.stringify` exactly as before, the "null" sentinel for reader/research is mirrored via `JSON.parse(getItem(...) ?? "null")`. 40 new tests in `web/src/prefs.test.ts`: namespace guard (every accessor writes only `akasha-*` and only the 26 inventoried keys), raw-string round-trip, boolean-flag exact bytes, numeric validation + NaN-fallback, nullable keys, JSON parse/stringify identity, corrupt-JSON handling per key (filters try/catch → []; colors/size-weights/reader/research throw matching current no-try/catch behavior), dynamic-slider round-trip, default-storage fallback, bytes-on-the-wire identity regression.

**Verifier result:** `npm test` 502/502 pass (33 files; +40 new vs prior 462); `npm run typecheck` clean (0 errors); `npm run build` green (vite dist 1,544.50 kB / 427.88 kB gzipped); `grep -c "localStorage" web/src/main.ts` -> 0. Stash@{0}/stash@{1}, `docs/plans/2026-07-07-010-feat-text-selection-to-voice-plan.md`, and `.scratchpad/` untouched. `i18n.ts` `akasha-lang` is out of scope (task said main.ts only). Manual smoke (set theme, arrangement, custom colors, reader geometry → reload → restored identically) deferred — no browser in this environment; verifier accepted on the test + build + grep evidence per R10 since the migration is one-line accessor substitutions whose byte-identity is covered by the bytes-on-the-wire test group.
...
**Inbox navigation correction:** checkpointed RM002/LAN work as `060a8b5`. Replaced the unintended two-tab Research/Inbox chrome with a persistent Inbox icon toggle immediately after research trash. Active previous/next/position now traverse the separate Inbox cursor; inactive controls retain research-history navigation. Trash remains research-only. Required serial gate passed 1016/1016 Vitest, typecheck, build, and 26/26 Playwright tests; independent focused validator passed. Live LAN dark-theme check confirmed the active toggle is visible with light `rgb(230, 237, 243)` background and dark `rgb(7, 10, 16)` foreground. Correction remains uncommitted for review.
